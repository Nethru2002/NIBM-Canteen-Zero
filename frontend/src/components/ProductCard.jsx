        ...
        <>
            <div className="group bg-white rounded-[2.5rem] p-3 shadow-premium border border-gray-100 hover:-translate-y-2 transition-all duration-500 animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
                <div className="relative h-52 w-full overflow-hidden rounded-[2rem] bg-gray-50">
                    <img src={product.image} alt={product.name} className="w-full h-full object-cover object-center transition-transform duration-700 group-hover:scale-110" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=400"; }} />
                    <button onClick={() => setShowDetails(true)} className="absolute top-4 right-4 p-2 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/40 transition-all opacity-0 group-hover:opacity-100"><Info size={16}/></button>
                ...
                <div className="px-3 py-5">
                    <div className="flex justify-between items-start mb-2">
                        <h3 onClick={() => setShowDetails(true)} className="text-lg font-bold text-gray-800 tracking-tight leading-tight w-2/3 cursor-pointer hover:text-nibmBlue">{product.name}</h3>
