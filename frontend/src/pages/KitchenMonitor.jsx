...
const KitchenMonitor = () => {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            ...
            case 'Preparing': return 'bg-nibmGold/10 text-nibmGold border-nibmGold/20 shadow-nibmGold/5';
        ...
        <div className="min-h-screen bg-slate-950 text-white font-sans p-8 overflow-x-hidden">
                    ...
                    <div className="text-left">
                        ...
                        <div className="flex items-center gap-3 mt-4">
                            ...
                            <div className="flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full border border-white/5">
                                <Activity size={12} className="text-nibmGold animate-pulse" />
                                <span className="text-[10px] font-black text-nibmGold uppercase tracking-widest">{orders.length} ACTIVE</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-40 opacity-20">
                    <div className="w-16 h-16 border-[6px] border-white/10 border-t-nibmGold rounded-full animate-spin mb-6"></div>
                    <p className="font-black uppercase tracking-[0.4em] text-[10px]">Accessing Stream...</p>
                </div>
            ) : (
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
                        ...
                        <div key={order._id} className="bg-white/[0.03] border border-white/5 rounded-[3.5rem] p-10 flex flex-col h-full hover:bg-white/[0.05] hover:border-white/10 transition-all relative overflow-hidden group shadow-2xl backdrop-blur-3xl animate-in zoom-in duration-500 text-left">
                            ...
                            <div className="flex justify-between items-start mb-10">
                                <div className="space-y-4">
                                    <span className={`px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-colors ${getStatusStyle(order.status)}`}>
                                        {order.status}
                                    </span>
                                    <div className="flex items-center gap-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-white/5 w-fit px-3 py-1.5 rounded-xl border border-white/5">
                                        <Clock size={12} className="text-nibmGold"/>
                                        <span>{calculateWait(order.createdAt)}</span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-[11px] font-black text-nibmGold uppercase tracking-[0.3em] mb-1 opacity-70">Token ID</p>
                                    <h2 className="text-7xl font-black tracking-tighter text-white drop-shadow-2xl">{order.tokenID}</h2>
                                </div>
                            </div>

                            <div className="flex-1 space-y-4 mb-12">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 px-1">Order Details</p>
                                {order.items.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-center bg-white/[0.02] p-5 rounded-[1.5rem] border border-white/[0.03] hover:border-white/10 transition-colors">
                                        <div className="flex items-center gap-5">
                                            <span className="w-10 h-10 bg-nibmGold text-nibmBlue rounded-2xl flex items-center justify-center font-black text-lg shadow-lg shadow-yellow-600/10">{item.quantity}</span>
                                            <span className="font-bold text-xl text-gray-100 tracking-tight">{item.name}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="mt-auto">
                                ...
                                {order.status === 'Preparing' && (
                                    <button 
                                        ...
                                        className="w-full bg-nibmRed text-white font-black py-6 rounded-[2rem] hover:bg-red-600 transition-all flex items-center justify-center gap-4 uppercase text-sm tracking-[0.2em] shadow-xl shadow-red-900/40 active:scale-95 group"
                                    >
                                        <CheckCircle2 size={20} className="group-hover:scale-110 transition-transform" /> Mark as Ready
                                    </button>
                                )}
                                {order.status === 'Ready' && (
                                    <div className="w-full py-6 text-center bg-green-500/10 border border-green-500/20 rounded-[2rem] text-green-400 font-black text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-3">
                                        <BellRing size={16} className="animate-bounce" /> Student Notified
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {orders.length === 0 && (
                        <div className="col-span-full py-40 text-center opacity-10 animate-pulse">
                            <ShoppingBag size={140} className="mx-auto mb-8 text-white" />
                            <h3 className="text-5xl font-black uppercase tracking-tighter text-white">Queue Clear</h3>
                            <p className="text-sm font-bold uppercase tracking-[0.5em] mt-4 text-white">Waiting for orders...</p>
                        </div>
                    )}
                </div>
            )}
