...
const AdminDashboard = () => {
    ...
    const alertAudio = useRef(new Audio(process.env.REACT_APP_NOTIFICATION_SOUND));
    ...
    return (
        <div className="h-screen w-full bg-[#f1f5f9] flex flex-col md:flex-row font-sans overflow-hidden text-left">
            <div className="w-full md:w-[60%] flex flex-col bg-[#f8fafc]">
                <div className="bg-[#0b3d91] p-8 text-white flex justify-between items-center shadow-lg z-10 text-left">
                    ...
                    <div className="flex items-center gap-4 text-white">
                        ...
                        <Link to="/admin/kitchen" className="relative flex items-center gap-3 bg-white/10 border border-white/20 px-5 py-2.5 rounded-2xl hover:bg-white/20 transition-all group text-white no-underline leading-none">
                            <ChefHat className="text-[#ffc600] group-hover:scale-110 transition-transform" size={24} />
                        ...
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Search..." className="bg-blue-900/40 border border-blue-400/30 rounded-full pl-9 pr-4 py-2 text-xs outline-none focus:ring-2 focus:ring-[#ffc600] w-40 transition-all text-white" value={searchTerm} onChange={(e) => setSearchBar(e.target.value)}/>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white border-b border-gray-100 p-4 flex justify-between items-center px-8 shadow-sm">
                    ...
                    <button onClick={handleGlobalReset} className="flex items-center gap-2 bg-slate-100 text-[#0b3d91] px-4 py-2 rounded-xl hover:bg-[#ffc600] transition-all font-black text-[9px] uppercase tracking-widest border border-slate-200 shadow-sm"><RotateCcw size={12} /> Global Reset</button>
                </div>

                <div className="p-8 overflow-y-auto no-scrollbar flex-1 space-y-8 pb-32">
                    <div>
                        <div className="flex items-center justify-between mb-6 px-1 text-left text-slate-900">
                            <h2 className="text-sm font-black text-[#0b3d91] uppercase tracking-widest flex items-center gap-2 text-left"><ShoppingBag size={16}/> Pending Pickups</h2>
                        </div>
                                ...
                                <div key={order._id} className="bg-[#ffc600] p-5 rounded-[2rem] flex justify-between items-center shadow-lg border border-white/50 animate-in fade-in zoom-in duration-300">
                                    <div className="flex items-center gap-4 text-left">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-[#0b3d91]/50 uppercase tracking-widest leading-none">{order.status}</span>
                                            <span className="text-[10px] font-bold text-[#d71920] uppercase mt-1">{order.items.length} Items</span>
                                        </div>
                                    </div>
                                    <button onClick={() => handleCollect(order._id)} className="bg-[#0b3d91] text-white p-3 rounded-2xl hover:bg-slate-900 transition-all shadow-lg"><CheckCircle2 size={20}/></button>
                                </div>
                            ))}
                            {pendingOrders.length === 0 && <p className="col-span-full text-center py-10 text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Queue Clear</p>}
                        </div>
                    </div>

                    <div>
                        <h2 className="text-sm font-black text-[#0b3d91] uppercase tracking-widest mb-6 px-1 text-left">Master Inventory</h2>
                            ...
                            {inventory.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase())).map(item => (
                                <div key={item._id} className={`flex items-center justify-between p-5 rounded-[2rem] border transition-all ${item.isAvailable ? 'bg-white border-gray-100 shadow-sm' : 'bg-gray-100 border-dashed border-gray-300 opacity-60'}`}>
                                    <div className="flex items-center gap-5">
                                        ...
                                        <div className="text-left">
                                            ...
                                            <div className="flex gap-2 mt-1">
                                                ...
                                                <span className="text-[9px] font-black text-slate-900 uppercase tracking-tighter font-mono italic text-left">LKR {item.price}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button onClick={() => handleDelete(item._id, item.name)} className="p-2.5 text-[#d71920] hover:bg-red-50 rounded-xl transition-all"><Trash2 size={16}/></button>
            ...
            <div className={`w-full md:w-[40%] bg-white shadow-[-20px_0_60px_rgba(0,0,0,0.03)] flex flex-col z-20 transition-all ${isEditing ? 'border-l-4 border-[#ffc600]' : ''}`}>
                <div className="p-10 text-center border-b border-gray-50 relative">
                    ...
                    <div className={`w-12 h-12 rounded-2xl mx-auto mb-3 flex items-center justify-center shadow-lg transition-all ${isEditing ? 'bg-[#ffc600] text-[#0b3d91] scale-110' : 'bg-slate-100 text-slate-300'}`}>
                        <PackagePlus size={24} />
                    </div>
                ...
                <form onSubmit={handleSubmit} className="px-10 pt-10 pb-20 space-y-6 overflow-y-auto no-scrollbar flex-1 text-slate-900">
                    <div className="grid grid-cols-2 gap-4 text-left">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Name</label>
                            <input type="text" name="name" required className="w-full bg-gray-50 p-4 rounded-2xl outline-none focus:ring-2 focus:ring-[#0b3d91] text-sm font-medium" value={formData.name} onChange={handleChange} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1"><Utensils size={12} className="text-[#0b3d91]"/> Category</label>
                            <select name="category" className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-[#0b3d91] text-sm appearance-none" value={formData.category} onChange={handleChange}>
                    ...
                    <div className="space-y-1.5 text-left">
                        ...
                        <textarea name="description" rows="2" required className="w-full bg-gray-50 p-4 rounded-2xl outline-none text-sm resize-none font-medium" value={formData.description} onChange={handleChange} />
                    </div>

                    <div className="grid grid-cols-3 gap-4 text-left">
                        <div className="space-y-1.5">
                            <label className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1"><BadgeDollarSign size={12} className="text-[#0b3d91]"/> Price</label>
                            <input type="number" name="price" className="w-full bg-gray-50 p-4 rounded-2xl text-center font-black text-[#0b3d91]" value={formData.price} onChange={handleChange} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1"><Clock size={12} className="text-[#0b3d91]"/> Prep</label>
                            <input type="number" name="prepTime" className="w-full bg-gray-50 p-4 rounded-2xl text-center font-black text-[#0b3d91]" value={formData.prepTime} onChange={handleChange} />
                        </div>
                        <div className="space-y-1.5 flex flex-col items-center">
                            <label className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-wider"><Flame size={12} className="text-[#d71920]"/> Spice</label>
                    ...
                    <div className="flex gap-4 text-left">
                        <div className="flex-1 space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Media</label>
                    ...
                    <button type="submit" disabled={loading} className={`w-full text-white font-black py-5 rounded-2xl shadow-xl transition-all flex items-center justify-center gap-3 uppercase tracking-widest ${isEditing ? 'bg-[#ffc600] text-[#0b3d91] hover:bg-yellow-500 shadow-yellow-100' : 'bg-[#d71920] hover:bg-red-700 shadow-red-100'}`}>
