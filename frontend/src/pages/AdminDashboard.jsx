...
import { 
    ...
    Power, Search, CalendarDays, RotateCcw, Edit3, Trash2, X
} from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    const handleSpiceClick = (level) => {
        setFormData(prev => ({ 
            ...prev, 
            spiceLevel: prev.spiceLevel === level ? 0 : level 
        }));
    };

    const fetchInventory = useCallback(async () => {
        try {
            const res = await axios.get(`${API_URL}/api/products/admin-list`);
    ...
    const handleGlobalReset = async () => {
        if(window.confirm("Initialize new day? All items will become Available.")) {
            const resetPromise = axios.post(`${API_URL}/api/products/daily-reset`);
            toast.promise(resetPromise, {
                loading: 'Resetting inventory...',
                success: () => { fetchInventory(); return 'Daily Session Started'; },
                error: 'Reset failed',
            });
        }
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!imageFile && !isEditing) return toast.error("Product image required");
        setLoading(true);
        ...
        try {
            ...
            toast.error(err.response?.data?.message || "Check fields"); 
        ...
        <div className="h-screen w-full bg-[#f1f5f9] flex flex-col md:flex-row font-sans text-slate-900 overflow-hidden">
            ...
            <div className="w-full md:w-[60%] flex flex-col bg-[#f8fafc]">
                <div className="bg-nibmBlue p-8 text-white flex justify-between items-center shadow-lg z-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-blue-200 text-[9px] font-bold tracking-[0.3em] uppercase mt-1 opacity-70">Operational Admin • US 23</p>
                        </div>
                    </div>
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Search catalog..." 
                            className="bg-blue-900/40 border border-blue-400/30 rounded-full pl-9 pr-4 py-2 text-xs outline-none focus:ring-2 focus:ring-nibmGold w-48 transition-all"
                ...
                <div className="bg-white border-b border-gray-100 p-4 flex justify-between items-center px-8 shadow-sm">
                    ...
                    <button onClick={handleGlobalReset} className="flex items-center gap-2 bg-slate-100 text-nibmBlue px-4 py-2 rounded-xl hover:bg-nibmGold transition-all font-black text-[9px] uppercase tracking-widest border border-slate-200">
                        <RotateCcw size={12} /> Global Inventory Reset
                    </button>
                </div>

                <div className="p-8 overflow-y-auto no-scrollbar flex-1 space-y-4 pb-24">
                        ...
                        <div key={item._id} className={`flex items-center justify-between p-5 rounded-[2rem] border transition-all ${item.isAvailable ? 'bg-white border-gray-100 shadow-sm' : 'bg-gray-100 border-dashed border-gray-300 opacity-60'}`}>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => handleToggle(item._id)}
                                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-black text-[10px] tracking-widest uppercase transition-all ${item.isAvailable ? 'bg-green-100 text-green-600' : 'bg-slate-200 text-slate-500'}`}>
                                    <Power size={14} /> {item.isAvailable ? 'Live' : 'Off'}
                                </button>
                            </div>
                        </div>
                    ))}
            ...
            <div className={`w-full md:w-[40%] bg-white shadow-[-20px_0_60px_rgba(0,0,0,0.03)] flex flex-col z-20 transition-all ${isEditing ? 'border-l-4 border-nibmGold' : ''}`}>
                <div className="p-10 text-center border-b border-gray-50 relative">
                    {isEditing && (
                        <button onClick={cancelEdit} className="absolute left-6 top-1/2 -translate-y-1/2 p-2 hover:bg-gray-100 rounded-full text-gray-400 transition-all"><X size={18}/></button>
                    )}
                ...
                <form onSubmit={handleSubmit} className="p-10 space-y-6 overflow-hidden flex-1">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <input type="text" name="name" required className="w-full bg-gray-50 p-4 rounded-2xl outline-none focus:ring-2 focus:ring-nibmBlue text-sm font-medium"
                        ...
                        <div className="space-y-1.5">
                            ...
                            <select name="category" className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-nibmBlue text-sm appearance-none"
                    ...
                    <div className="space-y-1.5">
                        ...
                        <textarea name="description" rows="2" required className="w-full bg-gray-50 p-4 rounded-2xl outline-none text-sm resize-none font-medium text-gray-600"
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <input type="number" name="price" className="w-full bg-gray-50 p-4 rounded-2xl text-center font-black text-nibmBlue"
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input type="number" name="prepTime" className="w-full bg-gray-50 p-4 rounded-2xl text-center font-black text-nibmBlue"
                        ...
                        <div className="space-y-1.5 flex flex-col items-center">
                            ...
                            <div className="flex gap-1.5 mt-3">
                                {[1, 2, 3].map(i => (
                                    <button key={i} type="button" onClick={() => handleSpiceClick(i)} 
                                        className={`text-sm transition-all ${formData.spiceLevel >= i ? 'scale-125' : 'opacity-10 grayscale'}`}>🌶️</button>
                                ))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4">
                        <div className="flex-1 space-y-1.5">
                            ...
                            <button type="button" onClick={() => fileInputRef.current.click()} 
                                className={`w-full border-2 border-dashed p-4 rounded-2xl font-bold text-[10px] tracking-widest uppercase flex items-center justify-center gap-2 transition-all ${imageFile ? 'bg-blue-600 border-blue-600 text-white' : 'bg-blue-50 border-blue-100 text-blue-600'}`}>
                                <Upload size={14} /> {imageFile ? 'Media Attached' : isEditing ? 'Update Media' : 'Upload Image'}
                            </button>
                            <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileChange} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider">Type</label>
                            <button type="button" onClick={() => setFormData(prev => ({...prev, isVeg: !prev.isVeg}))}
                                className={`h-[52px] px-6 rounded-2xl font-black text-[10px] tracking-widest uppercase border-2 transition-all flex items-center gap-2 ${formData.isVeg ? 'bg-green-600 border-green-600 text-white shadow-lg shadow-green-100' : 'bg-white border-gray-100 text-gray-400'}`}>
                    ...
                    <button type="submit" disabled={loading} 
                        className={`w-full text-white font-black py-5 rounded-2xl shadow-xl transition-all flex items-center justify-center gap-3 uppercase tracking-widest ${isEditing ? 'bg-nibmGold hover:bg-yellow-500' : 'bg-nibmRed hover:bg-red-700'}`}>
                    ...
                    {previewUrl && (
                        <div className="mt-4 p-4 bg-slate-50 rounded-[2rem] border border-gray-100 flex flex-col items-center">
                            <div className="flex items-center gap-2 text-gray-300 mb-3">
                                <Eye size={14} />
                                <span className="text-[9px] font-black uppercase tracking-widest leading-none">Integrity Check</span>
                            </div>
